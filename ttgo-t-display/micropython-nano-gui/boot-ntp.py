...
while (cnt < len(networks)) and (not iswlanready):
    ...
    start = time.ticks_ms()
    time.sleep(5) # this ensures a full network connect
...
def settime():
    ...
    tm = utime.gmtime(t)
    print('UTC time before synchronization：%s' %str(utime.gmtime()))
    machine.RTC().datetime((tm[0], tm[1], tm[2], tm[6] + 1, tm[3], tm[4], tm[5], 0))
    print('UTC time after synchronization：%s' %str(utime.gmtime()))
